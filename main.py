...
async def convert_link_to_button(message: types.Message) -> None:
    logger.info(f"Processing message in convert_link_to_button: '{message.text}' (chat_id={message.chat.id}, type={message.chat.type})")
    if not message.text:
        return
    ...
    logger.debug(f"Has Early: {has_early}, Has Fasol: {has_fasol}")
    ...
    if has_fasol:
        ...
        try:
            ...
            # Attempt to delete the original message, but don’t fail if it can’t
            try:
    ...
    # Prepare and send the output message with filter results (no buttons for "Early")
    first_line = text.split('\n')[0].strip()
