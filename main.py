...
CHECK_INTERVAL = 60  # Changed to 30 seconds from 300
...
class APISessionManager:
    def __init__(self):
        ...
        self.base_url = "https://gmgn.ai/defi/quotation/v1/tokens/sol"  # Old GMGN, not used anymore
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                try:
                    ...
                    logger.error(f"Error parsing CoinGecko response for CA {mint_address}: {e}")
                    return {"error": f"Invalid response parse for {mint_address}: {e}"}
...
async def growthcheck() -> None:
    ...
    for ca, channel_data in tokens_by_ca.items():
        ...
        for chat_id, data in channel_data.items():
            ...
            if growth_ratio >= 2.3 and time_diff_seconds <= 90 and ca not in notified_cas:
